...
export async function handleCalculateRequest(request: Request, contractVersion: "v1" | "v2") {
  ...
  if (request.method !== "POST")
    return methodNotAllowedResponse(ALLOWED_METHODS, resolveRequestId(request));
  ...
  const parsed = (
    contractVersion === "v2" ? CalculateV2InputSchema : CalculateInputSchema
  ).safeParse(body);
  if (!parsed.success) {
    return errorResponse(
  ...
  const normalized =
    contractVersion === "v2"
      ...
      : {
          engineInput: parsed.data as unknown as z.infer<typeof CalculateInputSchema>,
  ...
  if (contractVersion === "v2") {
    ...
    const structuredWarnings = toV2Warnings(
      [
        ...(Array.isArray(resultData.warnings) ? (resultData.warnings as string[]) : []),
        ...normalized.warnings,
      ],
    ...
    response.contract = {
      ...
      inputPresence: Object.fromEntries(
        Object.entries(normalized.structured ?? {}).map(([key, value]) => [
          key,
          value !== undefined,
        ]),
      ),
  ...
  return jsonResponse(response, { headers: { "X-Request-Id": requestId } });
}

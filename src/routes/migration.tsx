...
function MigrationPage() {
  ...
  async function compare(event: React.FormEvent) {
    ...
    try {
      ...
      if (!response.ok)
        throw new Error(body.error?.message || "The migration comparison could not be completed.");
      setResult(body);
    } catch (caught) {
      setError(
        caught instanceof Error
          ? caught.message
          : "The migration comparison could not be completed.",
      );
    } finally {
      setLoading(false);
    }
    ...
    <main className="min-h-screen bg-background text-foreground">
      ...
      <div className="mx-auto max-w-5xl space-y-8 px-4 py-8 sm:px-6">
        <section className="max-w-3xl space-y-3">
          <p className="text-sm font-medium text-primary">Migration review</p>
          <h1 className="text-3xl font-semibold tracking-normal">
            Compare approved V55 and V56 results
          </h1>
          <p className="text-base leading-7 text-muted-foreground">
            Review a stored, approved V55 baseline against the current V56 engine. This is a
            migration aid for known fixtures, not a general historical calculator.
          </p>
        </section>
        <form
          onSubmit={compare}
        ...
        >
          ...
          <Select value={fixtureId} onValueChange={setFixtureId}>
            ...
            <SelectContent>
              <SelectItem value="fixture-v19-007">
                Grad PLUS SOR reduction, Spring 5 credits
              </SelectItem>
            </SelectContent>
          </Select>
          <Button type="submit" disabled={loading}>
            {loading ? "Comparing..." : "Compare V55 and V56"}
        ...
        {comparison ? (
          <section className="space-y-5 rounded-lg border border-border bg-card p-5">
            <div>
              <p className="text-sm font-medium text-primary">Approved fixture comparison</p>
              <p className="mt-1 text-sm text-muted-foreground">
                {comparison.fixture?.description}
              </p>
              <p className="mt-2 text-xs text-muted-foreground">
                Release {result?.meta?.releaseId || "current"} · V56 authoritative:{" "}
                {comparison.v56?.authoritative ? "Yes" : "No"}
              </p>
            </div>
              ...
              <table className="w-full min-w-[560px] text-sm">
                <thead>
                  <tr className="border-b border-border text-left text-muted-foreground">
                    ...
                    <th className="px-2 py-2 text-right">V55</th>
                    <th className="px-2 py-2 text-right">V56</th>
                ...
                <tbody>
                  {Object.entries(comparison.changes || {}).map(([key, value]) => (
                    <tr key={key} className="border-b border-border/50">
                      ...
                      <td className="px-2 py-2 text-right">{money(value.v55)}</td>
                      <td className="px-2 py-2 text-right">{money(value.v56)}</td>
            ...
            <p className="text-sm text-muted-foreground">
              Changed outputs: {comparison.changedMetrics?.join(", ") || "None"}. Source references:{" "}
              {comparison.fixture?.sourceRefs.join(", ") || "Not listed"}.
            </p>
            <a
              ...
              rel="noreferrer"
            >
              View public source <ExternalLink className="h-4 w-4" />
            </a>
          </section>
        ) : null}
      </div>

      ...
      POST: async ({ request }) => {
        ...
        if (!parsed.success)
          return errorResponse(
            ...
            "Student estimate inputs are incomplete or invalid.",
            parsed.error.issues,
            undefined,
            requestId,
          );
        ...
        base.terms.term1 = {
          ...base.terms.term1,
          enabled: true,
          ftCredits: value.fullTimeCreditsPerTerm,
          enrolledCredits: value.fallCredits,
        };
        base.terms.term2 = {
          ...base.terms.term2,
          enabled: true,
          ftCredits: value.fullTimeCreditsPerTerm,
          enrolledCredits: value.springCredits,
        };
        for (const key of [
          "term3",
          "term4",
        ...
        return jsonResponse(
          {
            ...
            disclaimer:
              "This is an estimate, not an award, approval, or guarantee. The school must verify full-time definitions, COA, other aid, grade level, dependency, SAP, aggregate limits, and all other Direct Loan eligibility requirements.",
            unsupported: [
              "single-term",
              "summer",
              "modules",
